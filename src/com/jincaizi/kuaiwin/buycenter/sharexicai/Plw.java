...
public class Plw extends FragmentActivity implements OnClickListener {
	protected static final String TAG = "Plw";
	...
	private void _loadFragment() {
		...
		if (!TextUtils.isEmpty(intent.getAction())
			...
			startType = 1;
			_showFragments(PlwNormalFragment.TAG);
			...
			String ballStr = intent.getStringExtra(PlsPick.BALL);
				_showFragments(PlwNormalFragment.TAG);
				plsType = PlsType.OTHER;
				PlwNormalFragment zuFragment = ((PlwNormalFragment) mCurrentFragment);
	...
	private void _showFragments(String fragmentTag) {
		...
		if (mFragment == null) {
			if (fragmentTag.equals(PlwNormalFragment.TAG)) {
				mFragment = new PlwNormalFragment();
			}
	...
	private void _initBallStr() {
		PlwNormalFragment zhixuanFragment = ((PlwNormalFragment) mCurrentFragment);
	...
	public void onClick(View v) {
		...
		case R.id.my_pls_shake_pick:
			PlwNormalFragment shakeFragment = ((PlwNormalFragment) mCurrentFragment);
		...
		case R.id.right_footer_btn:
			...
			if (mCount < 1) {
				...
				Intent plwPick = new Intent();
				plwPick.putExtra(PlsPick.BETTYPE, plsType.toString());
                plwPick.putExtra("qihao", mQihao);
					plwPick.putStringArrayListExtra(PlsPick.BALL,
							((PlwNormalFragment) mCurrentFragment)
									.getPlsResultList());

				if (startType == 0) {
					plwPick.setClass(Plw.this, PlwPick.class);
					startActivity(plwPick);
				} else if (startType == 1) {
					setResult(RESULT_OK, plwPick);
				} else if (startType == 2) {
					setResult(RESULT_OK, plwPick);
				}
				if(mc != null) {
		...
		myRequestHandle = JinCaiZiHttpClient.post(this, url,
				new AsyncHttpResponseHandler() {
					...
					public void onSuccess(int statusCode, Header[] headers,
						...
						try {
							String charset;
							if(Utils.isCmwapNet(Plw.this)) {
								charset = "utf-8";
							} else {
								charset = "gb2312";
							}
							String jsonData = new String(responseBody, charset);
							Log.d(TAG, "pl5 qihao detail = " + jsonData);
					...
					public void onFailure(int statusCode, Header[] headers,
						...
						mQihaoView.setText("获取期号失败，点击重新获取");
						mTimeDiffView.setText("");
						Toast.makeText(Plw.this, "期号获取失败", Toast.LENGTH_SHORT).show();
					}
				});
	}
