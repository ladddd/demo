...
public class SmartFollow extends Activity implements OnClickListener, OnCheckedChangeListener{
	...
	private TextView periodSub;
	private TextView periodAdd;
	private EditText periodValue;
	private TextView timeStartSub;
	private TextView timeStartAdd;
    ...
    private void _findViews() {
    	...
    	mFooterRight = (TextView)findViewById(R.id.right_footer_btn);
    	mHeaderLeft = (ImageView)findViewById(R.id.touzhu_leftmenu);
    	mBetMoney = (TextView)findViewById(R.id.bet_zhushu);
    	mSettingBtn = (Button)findViewById(R.id.smartfollow_bt);
    ...
    private void _setListener() {
    	...
    	mFooterRight.setOnClickListener(this);
    	mQihaoView.setOnClickListener(this);
    	mHeaderLeft.setOnClickListener(this);
    ...
    private void _updateFooterValue() {
    	if(mData.size() == 0) {
    		mBetMoney.setText("0元");
    	} else {
    	    mBetMoney.setText(mData.get(mData.size()-1).touruAmount+"元");
    	}
    }
	@Override
	public void onClick(View v) {
		...
		case R.id.times_start_add:
			...
			if(timestartadd < 1) {
				timeStartValue.setText("1");
			} else if(timestartadd > 9999) {
				timeStartValue.setText("9999");
			} else  {
    ...
    private void _findDialogViews(View view) {
    	periodSub = (TextView)view.findViewById(R.id.period_sub);
    	periodAdd = (TextView)view.findViewById(R.id.period_add);
    	periodValue = (EditText)view.findViewById(R.id.period_time_tv);
    	
    	timeStartSub = (TextView)view.findViewById(R.id.times_start_sub);
    	timeStartAdd = (TextView)view.findViewById(R.id.times_start_add);
...
//		}
    ...
    private void _setDialogListeners() {
    	...
    	periodValue.addTextChangedListener(new MyPeriodWatcher());
    	timeStartValue.addTextChangedListener(new MyTimeStartWatcher());
    ...
    private void _generateSubPlanC() {
		...
		for(int i=0; i<mZhuiHaoNums; i++) {
			...
			if(percent.length == 1 && yinli[0] < profitBonusValue) {
				isRecompute = true;
				targetAward = perAward[0];
			}else if(percent.length == 2 && Math.min(yinli[0], yinli[1]) < profitBonusValue) {
				isRecompute = true;
				targetAward = perAward[0];
			}
			if(isRecompute) {
