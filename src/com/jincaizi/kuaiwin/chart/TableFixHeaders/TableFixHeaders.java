package com.jincaizi.kuaiwin.chart.TableFixHeaders;
...
import com.jincaizi.kuaiwin.chart.TableFixHeaders.adapters.TableAdapter;
...
public class TableFixHeaders extends ViewGroup {
    ...
    private TableFixHeaders delegateTable;

    public boolean isPositiveScroll() {
        return positiveScroll;
    }

    public void setPositiveScroll(boolean positiveScroll) {
        this.positiveScroll = positiveScroll;
    }

    private boolean positiveScroll = true;
	...
	protected void onLayout(boolean changed, int l, int t, int r, int b) {
		if (needRelayout || changed) {
			...
			if (adapter != null) {
                ...
                addLineView(widths[0], heights[0], sumArray(widths, 1, widths.length-1),
...
//		firstColumn = values[1];
	...
	private void addShadow(ImageView imageView, int l, int t, int r, int b) {
		...
		addView(imageView, getChildCount() - 4);
	}
	...
	private void addTableView(View view, int row, int column) {
		if (row == -1 && column == -1) {
			...
			addView(view, getChildCount() - 6);
		} else {
			addView(view, 0);
		}
	}

    public void setDelegateTable(TableFixHeaders table)
    {
        delegateTable = table;
    }
